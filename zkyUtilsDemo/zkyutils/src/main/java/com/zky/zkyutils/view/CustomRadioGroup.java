...
public class CustomRadioGroup extends LinearLayout {
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        if (child instanceof RadioButton) {
            ...
            setRadionButton(button);
        } else if (child instanceof ViewGroup) {
            ViewGroup group = (ViewGroup) child;
            int len = group.getChildCount();
            for (int i = 0; i < len; i++) {
                if (group.getChildAt(i) instanceof RadioButton) {
                    setRadionButton((RadioButton) group.getChildAt(i));
                }
            }
        }
    ...
    private void setRadionButton(RadioButton button) {
        if (button.isChecked()) {
            mProtectFromCheckedChange = true;
            if (mCheckedId != -1) {
    ...
    private class PassThroughHierarchyChangeListener implements
        ...
        public void onChildViewAdded(View parent, View child) {
            if (parent == CustomRadioGroup.this && child instanceof RadioButton) {
                ...
                for (int i = 0; i < len; i++) {
                    if (group.getChildAt(i) instanceof RadioButton) {
                        addRadioButton((RadioButton) group.getChildAt(i));
                    }
                }
            }
        ...
        private void addRadioButton(RadioButton btn) {
            int id = btn.getId();
            // generates an id if it's missing
            if (id == View.NO_ID) {
                id = btn.hashCode();
        ...
        public void onChildViewRemoved(View parent, View child) {
            if (parent == CustomRadioGroup.this && child instanceof RadioButton) {
                ...
                for (int i = 0; i < len; i++) {
                    if (group.getChildAt(i) instanceof RadioButton) {
                        ((RadioButton) group.getChildAt(i)).setOnCheckedChangeListener(
                                null);
                    }
                }
            }
